...
export function renderChatsList() {
    ...
    // Оптимизация против "моргания": карточки чатов переиспользуются, а не пересобираются через
    // innerHTML на каждое изменение — обновляем только текст/время/непрочитанное и переставляем.
...
const messages = chat.messages ? Object.entries(chat.messages).map(([id, m]) => ({ id, ...m })).sort((a, b) => a.createdAt - b.createdAt) : [];
    ...
    // Не пересобираем всю ленту через innerHTML при любом изменении (это гасило старые узлы —
    // аватарки/картинки/iframe — и на миг показывало пустой экран). Ниже переиспользуем уже
    // отрисованные сообщения и дорисовываем только новое/изменившееся.
    const prevParts = state.renderedChatState.msgSigParts || [];

    // Если поменялись только readReceipts (собеседник прочитал) — точечно обновляем галочки/время
    // у своих сообщений, не трогая остальной DOM, вместо полной перерисовки.
    ...
    if (canAppendOnly) {
        ...
        // Firebase иногда зовёт onValue дважды на одну запись (сначала локально-оптимистично,
        // потом с сервера) — проверяем, нет ли уже узла с таким id, чтобы не задваивать сообщение.
...
// Перевод — напрямую в браузере через MyMemory (без ключа), а не через Cloud Function, т.к.
// *.cloudfunctions.net не всегда доступен из России; готовый перевод кэшируется в Firebase.
// Необязательно: впиши email — анонимно лимит 5000 слов/сутки на IP, с email — 50000;
// сначала пробуем без email, при "квота исчерпана" автоматически повторяем с ним.
...
// ГМ может писать CSS (санитайзер/скоупер — см. sanitizeAndScopeWikiCss в utils.js) и добавлять
// виджеты (HTML+CSS+JS), исполняемые у всех участников группы. Виджет — недоверенный код, живёт
// в <iframe sandbox="allow-scripts"> без allow-same-origin: opaque-происхождение, нет доступа
// к куки/localStorage/DOM приложения и к сети (connect-src 'none' в CSP).
...
export function renderWikiPost() {
    ...
    // Весь чат подписан на общий узел 'chats' в базе, поэтому раньше renderWikiPost() вызывался
    // при любом изменении в любом чате (например, индикатор "печатает..." в другом чате) и
    // каждый раз пересоздавал галерею и iframe виджета — отсюда моргание. Теперь сравниваем
    // "подпись" поста: если не изменилась — выходим, не трогая DOM. Подпись разбита на "тяжёлую"
    // (картинки/виджет/автор — их пересоздание и моргает) и "лёгкую" (комментарии — просто текст).
