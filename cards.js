...
window.editCard = function (id) {
    ...
    document.getElementById('card-cooldown').value = c.cooldown ?? 2;
...
document.getElementById('btn-cancel-edit-card').onclick = function () {
    ...
    ['card-name', 'card-image', 'card-mana', 'card-attack', 'card-health', 'card-effect', 'card-effect-value', 'card-cooldown'].forEach(id => {
...
document.getElementById('btn-add-card').onclick = function () {
    ...
    const cooldown = parseInt(document.getElementById('card-cooldown').value, 10) || 2;
...
window.renderAdminArenasList = function() {
    const el = document.getElementById('admin-arenas-list');
    if (!el) return;

    if (!state.arenasData || !state.arenasData.length) {
        el.innerHTML = '<div style="color:var(--text-secondary);font-size:13px;">Арен пока нет. Будет использоваться стандартное поле.</div>';
        return;
    }

    el.innerHTML = state.arenasData.map(a => `
        <div class="admin-item">
            ${a.bgUrl ? `<img src="${a.bgUrl}" class="admin-item-thumb" onerror="this.style.display='none'">` : `<div class="admin-item-thumb cover-fallback small" style="background:#4a3d2e;">🗺️</div>`}
            <div class="admin-item-info">
                <div class="admin-item-title">${escapeHtml(a.name || 'Арена')}</div>
                <div class="admin-item-sub">Музыка: ${a.bgmUrl ? '✅' : '❌'} | Эффект: ${a.effectType || 'Нет'}</div>
            </div>
            <div class="admin-item-actions">
                <button class="icon-btn" onclick="window.editArena('${a.id}')">✏️</button>
                <button class="icon-btn danger" onclick="window.deleteArena('${a.id}')">🗑</button>
            </div>
        </div>
    `).join('');
};

window.editArena = function(id) {
    const a = state.arenasData.find(x => x.id === id);
    if (!a) return;

    state.editingArenaId = id;
    document.getElementById('arena-name').value = a.name || '';
    document.getElementById('arena-image').value = a.bgUrl || '';
    document.getElementById('arena-bgm').value = a.bgmUrl || '';
    document.getElementById('arena-effect-type').value = a.effectType || '';
    document.getElementById('arena-effect-value').value = a.effectValue || '';
    
    document.getElementById('arena-form-heading').textContent = 'Редактировать Арену';
    document.getElementById('btn-add-arena').textContent = 'Сохранить';
    document.getElementById('btn-cancel-edit-arena').classList.remove('hidden');
    document.getElementById('arena-name').scrollIntoView({ behavior: 'smooth' });
};

window.cancelEditArena = function() {
    state.editingArenaId = null;
    document.getElementById('arena-name').value = '';
    document.getElementById('arena-image').value = '';
    document.getElementById('arena-bgm').value = '';
    document.getElementById('arena-effect-type').value = '';
    document.getElementById('arena-effect-value').value = '';
    
    document.getElementById('arena-form-heading').textContent = 'Создать Поле Боя (Арену)';
    document.getElementById('btn-add-arena').textContent = 'Добавить арену';
    document.getElementById('btn-cancel-edit-arena').classList.add('hidden');
};

window.saveArena = function() {
    const name = document.getElementById('arena-name').value.trim();
    const bgUrl = document.getElementById('arena-image').value.trim();
    const bgmUrl = document.getElementById('arena-bgm').value.trim();
    const effectType = document.getElementById('arena-effect-type').value;
    const effectValue = parseInt(document.getElementById('arena-effect-value').value, 10) || 0;

    if (!name) return tg.showAlert('Укажи название арены');

    const data = { name, bgUrl, bgmUrl, effectType, effectValue };

    if (state.editingArenaId) {
        update(ref(state.db, 'arenas/' + state.editingArenaId), data).then(() => {
            window.cancelEditArena();
            tg.showPopup({ title: 'Готово', message: 'Арена обновлена', buttons: [{ type: 'ok' }] });
        }).catch(err => tg.showAlert('Ошибка: ' + friendlyDbError(err)));
    } else {
        push(ref(state.db, 'arenas'), { ...data, createdAt: Date.now() }).then(() => {
            window.cancelEditArena();
            tg.showPopup({ title: 'Супер!', message: 'Новая арена создана', buttons: [{ type: 'ok' }] });
        }).catch(err => tg.showAlert('Ошибка: ' + friendlyDbError(err)));
    }
};

window.deleteArena = function(id) {
