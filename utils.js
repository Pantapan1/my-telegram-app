...
export function colorFor(str) {
        ...
        // Хэширует пароль (SHA-256, hex) перед сохранением в базу, чтобы логин/пароль (auth_users)
        // не хранились в базе открытым текстом. Используется и при регистрации на стартовом экране,
        // и при привязке логина/пароля к существующему аккаунту в настройках профиля.
        // Если Web Crypto недоступен (например, страница открыта не по HTTPS), возвращает пароль как
        // есть — это резервный случай для совместимости, а не рекомендуемый режим работы.
