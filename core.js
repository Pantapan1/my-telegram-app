...
export function startFirebaseListeners() {
    ...
    // Общий предохранитель: если что-то зависло (плохая сеть, ошибка правил Firebase, юзер ещё
    // не залогинен и т.п.) — не держим экран загрузки дольше 10 секунд ни при каких условиях,
    // просто открываем то, что за ним (форму входа или приложение с тем, что успело прийти).
    setTimeout(hideAppSplash, 10000);

    // Кнопка "Пропустить загрузку" — на случай, если человек не хочет ждать все 10 секунд
    // до автоматического скрытия ниже.
    setTimeout(() => {
