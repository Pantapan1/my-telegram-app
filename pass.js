...
export function passLevelForXp(xp) {
    ...
    for (const l of levels) { 
        if (xp >= (l.xpRequired || 0)) lvl = l.level; 
        else break; 
    }
    return lvl;
}

// Гарантирует, что прогресс пасса пользователя соответствует текущему сезону;
// при смене месяца сбрасываются только уровни и XP, а все открытые награды (обои, титулы, питомцы) сохраняются навсегда.
export function ensurePassSeason() {
    ...
    if (!p || p.season !== season) {
        // 1. Сохраняем все накопленные награды инвентаря
        const existingUnlocked = (p && p.unlocked) ? { ...p.unlocked } : {};

        // 2. Автовосстановление: если обои или другие предметы были надеты, но стёрлись в прошлых сезонах
        const eq = (me && me.equipped) || {};
        if (eq.passChatBg) {
            existingUnlocked.chatbgs = existingUnlocked.chatbgs || {};
            const val = eq.passChatBg;
            if (!Object.values(existingUnlocked.chatbgs).includes(val)) {
                existingUnlocked.chatbgs['rec_' + Date.now().toString(36)] = val;
            }
        }
        if (eq.passTitle) {
            existingUnlocked.titles = existingUnlocked.titles || {};
            const val = eq.passTitle;
            if (!Object.values(existingUnlocked.titles).includes(val)) {
                existingUnlocked.titles['rec_' + Date.now().toString(36)] = val;
            }
        }
        if (eq.passPet) {
            existingUnlocked.pets = existingUnlocked.pets || {};
            const val = eq.passPet;
            if (!Object.values(existingUnlocked.pets).includes(val)) {
                existingUnlocked.pets['rec_' + Date.now().toString(36)] = val;
            }
        }
        if (eq.passOverlay) {
            existingUnlocked.overlays = existingUnlocked.overlays || {};
            const val = eq.passOverlay;
            if (!Object.values(existingUnlocked.overlays).includes(val)) {
                existingUnlocked.overlays['rec_' + Date.now().toString(36)] = val;
            }
        }
        ...
        // 3. Сохраняем VIP-статус, если он ещё действует
        if (p && p.vipUntil && p.vipUntil > Date.now()) {
            fresh.vipUntil = p.vipUntil;
        }
...
const openPassBtn = document.getElementById('btn-open-pass');
if (openPassBtn) openPassBtn.onclick = () => goToSubpage('pass.html');

const passPetWidget = document.getElementById('pass-pet-widget');
if (passPetWidget) {
    passPetWidget.onclick = function() {
